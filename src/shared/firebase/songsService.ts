...
function toSong(id: string, data: Record<string, unknown>): Song {
  ...
  return {
    ...
    generatedLyrics: (data.generatedLyrics as string | null) ?? null,
    melody: (data.melody as string | null) ?? null,
    timeline: (data.timeline as Song['timeline']) ?? createInitialTimeline(),
    messages: (data.messages as ChatMessage[]) ?? [],
    storySource: (data.storySource as Song['storySource']) ?? null,
    prologueStory: (data.prologueStory as string | null) ?? null,
    productionPhase: (data.productionPhase as Song['productionPhase']) ?? 'story_select',
    lyricsRegenCount: (data.lyricsRegenCount as number) ?? 0,
    lyricsConfirmedAt: (data.lyricsConfirmedAt as string | null) ?? null,
    productionReadyAt: (data.productionReadyAt as string | null) ?? null,
    videoUrl: (data.videoUrl as string | null) ?? null,
    audioUrl: (data.audioUrl as string | null) ?? null,
    lyricsFlowPhase: (data.lyricsFlowPhase as Song['lyricsFlowPhase']) ?? null,
    videoTier: (data.videoTier as Song['videoTier']) ?? null,
...
export async function firebaseCreateSong(data: CreateSongRequest) {
  ...
  const payload = {
    ...
    storySource: null,
    prologueStory: null,
    productionPhase: 'story_select' as const,
    lyricsRegenCount: 0,
    lyricsConfirmedAt: null,
...
export async function firebaseUpdateSong(id: string, data: UpdateSongRequest) {
  ...
  const patch = {
    ...
    generatedLyrics: data.generatedLyrics ?? (current.generatedLyrics as string | null) ?? null,
    melody: data.melody ?? derived.melody,
    timeline: data.timeline ?? derived.timeline,
    messages: nextMessages,
    storySource: data.storySource ?? (current.storySource as Song['storySource']) ?? null,
    prologueStory: data.prologueStory ?? (current.prologueStory as string | null) ?? null,
    productionPhase: data.productionPhase ?? (current.productionPhase as Song['productionPhase']) ?? 'story_select',
    lyricsRegenCount: data.lyricsRegenCount ?? (current.lyricsRegenCount as number) ?? 0,
    lyricsConfirmedAt: data.lyricsConfirmedAt ?? (current.lyricsConfirmedAt as string | null) ?? null,
    productionReadyAt: data.productionReadyAt ?? (current.productionReadyAt as string | null) ?? null,
    videoUrl: data.videoUrl ?? (current.videoUrl as string | null) ?? null,
    audioUrl: data.audioUrl ?? (current.audioUrl as string | null) ?? null,
    lyricsFlowPhase: data.lyricsFlowPhase ?? (current.lyricsFlowPhase as Song['lyricsFlowPhase']) ?? null,
    videoTier: data.videoTier ?? (current.videoTier as Song['videoTier']) ?? null,
    updatedAt: now,
  };
